...
import { mediaUrl, type TeamMemberCrm, type TeamMemberInput } from "@/lib/crmApi";
...
export default function TeamMemberForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      await onSubmit({ name, role, bio, phone, email, order, is_active: isActive, photo });
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        <label className="mb-1 block text-xs text-foreground/50">Должность</label>
        <input
          required
          type="text"
          value={role}
      ...
      <div>
        <label className="mb-1 block text-xs text-foreground/50">О себе</label>
        <textarea
          value={bio}
      ...
      <div>
        <label className="mb-1 block text-xs text-foreground/50">Фото</label>
        {initial?.photo && !photo && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={mediaUrl(initial.photo)} alt="" className="mb-2 h-20 w-20 rounded-full object-cover" />
        )}
      ...
      <div className="flex flex-wrap gap-3">
        <div>
          <label className="mb-1 block text-xs text-foreground/50">Телефон (необязательно)</label>
          <input
            type="text"
            value={phone}
        ...
        <div>
          <label className="mb-1 block text-xs text-foreground/50">Email (необязательно)</label>
          <input
            type="email"
            value={email}
